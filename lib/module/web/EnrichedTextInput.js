...
import { CustomEmojiNode, customEmojiKey } from "./customEmojis/CustomEmoji.js";
...
import { useOnCaretChange } from "./tiptapWatchers/useOnCaretChange.js";
...
function plainTextLength(doc) {
  ...
  doc.descendants(node => {
    ...
    length += node.isText ? node.text.length : node.type.name === 'customEmoji' ? node.attrs.shortcode.length : node.isLeaf ? 1 : 0;
  });
  return length;
}
export const EnrichedTextInput = ({
  ...
  const lengthLimit = useStableRef({
    maxPlainTextLength,
    onMaxLengthExceeded
  });
  const acceptsDocument = useCallback((doc, previous) => {
    const {
      maxPlainTextLength: maxLength,
      onMaxLengthExceeded: exceeded
    } = lengthLimit.current;
    if (maxLength < 0) return true;
    const length = plainTextLength(doc);
    if (length <= maxLength || length <= plainTextLength(previous)) return true;
    exceeded?.(adaptWebToNativeEvent(null, {
      maxLength
    }));
    return false;
  }, [lengthLimit]);
  ...
  const extensions = useMemo(() => [Extension.create({
    name: 'plainTextLengthLimit',
    addProseMirrorPlugins: () => [new Plugin({
      filterTransaction: (tr, state) => !tr.docChanged || tr.getMeta('lengthLimitHydration') || acceptsDocument(tr.doc, state.doc)
    })]
  }), Document, Paragraph, Text, History, EnrichedBold, EnrichedItalic, EnrichedUnderline, EnrichedStrike, EnrichedCode, EnrichedLink.configure({
    getLinkRegex: () => linkEmitterRef.current.linkRegex
  }), EnrichedImage, CustomEmojiNode.configure({
    getCatalog: () => emojiCatalogRef.current ?? [],
    onError: event => emojiErrorRef.current?.(event)
  }), EnrichedMention, EnrichedHeading, EnrichedBlockquote, EnrichedCodeBlock, EnrichedListItem, EnrichedCheckboxItem, EnrichedUnorderedList, EnrichedOrderedList, EnrichedCheckboxList, EnrichedTextAlign, StripMarksInCodeBlockPlugin, StripMarksOnImagePlugin, StripBoldInStyledHeadingsPlugin.configure({
  ...
  })], [placeholder, emojiCatalogRef, emojiErrorRef, htmlStyleRef, mentionIndicatorsRef, textShortcutsRef, acceptsDocument]);
  const editor = useEditor({
    ...
    onCreate: ({
      ...
      _editor.chain().setMeta('lengthLimitHydration', true).setContent(tiptapContent ?? '').run();
    },
    onFocus: ({
      event
    }) => {
    ...
    editorProps: {
      ...
      handleDOMEvents: {
        beforeinput: (view, event) => {
          if (lengthLimit.current.maxPlainTextLength < 0 || !event.cancelable || !event.inputType.startsWith('insert') || event.data == null) return false;
          const target = event.getTargetRanges?.()[0];
          const from = target ? view.posAtDOM(target.startContainer, target.startOffset) : view.state.selection.from;
          const to = target ? view.posAtDOM(target.endContainer, target.endOffset) : view.state.selection.to;
  ...
  useEffect(() => {
    if (editor && !editor.isDestroyed) editor.view.dispatch(editor.state.tr.setMeta(customEmojiKey, true));
  }, [editor, customEmojis]);
  useEffect(() => {
  ...
  const caretContainer = useRef(null);
  ...
  useImperativeHandle(ref, () => ({
    ...
    setValue: value => editor.chain().setMeta('lengthLimitHydration', true).setContent(prepareHtmlForTiptap(value, useHtmlNormalizerRef.current, sanitizationConfigRef.current)).run(),
  ...
  }), [editor, acceptsDocument, mentionIndicatorsRef, useHtmlNormalizerRef, sanitizationConfigRef, onPasteRef]);

...
export function useOnCaretChange(editor, container, onCaretChange) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      callback.current?.(adaptWebToNativeEvent(null, {
        x: rect.left - bounds.left,
        y: rect.top - bounds.top,
        width: rect.right - rect.left,
        height: rect.bottom - rect.top,
        visible: selection.empty && rect.bottom > rect.top && rect.top >= bounds.top && rect.bottom <= bounds.bottom && rect.left >= bounds.left && rect.left <= bounds.right
      }));
    };
    const schedule = () => {
...
//# sourceMappingURL=useOnCaretChange.js.map
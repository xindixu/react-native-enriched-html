...
export const EnrichedTextInput = ({
  ...
  const pendingPastes = useRef(new Map());
  ...
  useImperativeHandle(ref, () => ({
    ...
    completePaste: (requestId, html) => {
      const view = nativeRef.current;
      if (!view || pendingPastes.current.has(requestId)) return Promise.resolve(false);
  ...
  return /*#__PURE__*/_jsx(EnrichedTextInputNativeComponent, {
    ...
    onCustomEmojiError: onCustomEmojiError ? ({
      nativeEvent
    }) => onCustomEmojiError(nativeEvent) : undefined,
    editable: editable,
    autoFocus: autoFocus,

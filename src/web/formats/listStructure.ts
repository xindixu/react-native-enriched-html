...
export const ITEM_CONTENT =
  'paragraph (unorderedList | orderedList | checkboxList)*';
export const children = (node: Node): Node[] =>
  Array.from({ length: node.childCount }, (_, index) => node.child(index));
const itemDepth = ($pos: ResolvedPos) => {
  for (let depth = $pos.depth; depth > 0; depth--) {
    if (['listItem', 'checkboxItem'].includes($pos.node(depth).type.name))
      return depth;
  }
  return 0;
};
...
export function selectedList(selection: Selection) {
  ...
  if (
    !depth ||
    itemDepth($to) !== depth ||
    $from.before(depth - 1) !== $to.before(depth - 1)
  )
    return null;
  return {
    depth: depth - 1,
...
export function outdentList(tr: Transaction): boolean {
  ...
  if (
    depth > 1 &&
    ['listItem', 'checkboxItem'].includes($from.node(depth - 1).type.name)
  ) {
    ...
    const converted = lifted.map((item) =>
      item.type === itemType ? item : itemType.create(null, item.content)
    );
    const tail = items.slice(to);
    const last = converted[converted.length - 1]!;
    const trailing = [
      ...(tail.length ? [list.copy(Fragment.from(tail))] : []),
      ...parentContent.slice(listIndex + 1),
    ];
    converted[converted.length - 1] = last.copy(
      last.content.append(Fragment.from(trailing))
    );
    const outerItems = children(outer);
    outerItems.splice(
      parentIndex,
      1,
      parent.copy(Fragment.from(remaining)),
      ...converted
    );
    replaceList(tr, $from.before(depth - 2), outer, [
      outer.copy(Fragment.from(outerItems)),
    ]);
  } else {
    const replacement = [
      ...(from ? [list.copy(Fragment.from(items.slice(0, from)))] : []),
      ...lifted.flatMap(children),
      ...(to < items.length ? [list.copy(Fragment.from(items.slice(to)))] : []),
    ];
